...
TARGET="$HOME/Magisk/pc_magisk/boot_patch.sh"

if [[ -f "$TARGET" ]]; then

  # Transfer execution flow to foobar.sh (replaces this process)
    read -e -p "Drag & drop your boot.img : " file
    eval file=$file
    exec sh "$TARGET" $file
fi

# This runs only if foobar.sh doesn’t exist
...
copy_files() {
    ...
    cp $HOME/Magisk/wokdir/assets/stub.apk $HOME/Magisk/pc_magisk/stub.apk
    cp $HOME/Magisk/wokdir/lib/armeabi-v7a/libmagisk.so $HOME/Magisk/pc_magisk/magisk
    cp $HOME/Magisk/wokdir/lib/armeabi-v7a/libmagiskinit.so $HOME/Magisk/pc_magisk/magiskinit
    cp $HOME/Magisk/wokdir/lib/armeabi-v7a/libinit-ld.so $HOME/Magisk/pc_magisk/init-ld
...
install_dependencies() {
    ...
    case $OS in
        "Ubuntu" | "Linux Mint")
            sudo apt update
            sudo apt install dos2unix unzip ed curl -y
            ;;
        "Fedora")
    ...
    programs=("dos2unix" "unzip" "curl" "ed")
